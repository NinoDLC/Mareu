...
public final class utils {
    ...
    public static void initMeetings(@NonNull MeetingsRepository repo) {
        repo.createMeeting(new Meeting(repo.getNextMeetingId(), "marc@lamzone.fr", new HashSet<>(Collections.singletonList("claire@nerdzherdz.org")), "Daily meetup", LocalDateTime.of(now.getYear(), now.getMonthValue(), now.getDayOfMonth(), 8, 30, 0), LocalDateTime.of(now.getYear(), now.getMonthValue(), now.getDayOfMonth(), 9, 35, 0), MeetingRoom.values()[3]));
        repo.createMeeting(new Meeting(repo.getNextMeetingId(), "tedy@buymore.fr", new HashSet<>(Arrays.asList("claire@nerdzherdz.com", "jack@lamzone.fr", "jack@lamzone.net")), "Project xXx", LocalDateTime.of(now.getYear(), now.getMonthValue(), now.getDayOfMonth(), 16, 15, 0), LocalDateTime.of(now.getYear(), now.getMonthValue(), now.getDayOfMonth(), 16, 40, 0), MeetingRoom.values()[1]));
